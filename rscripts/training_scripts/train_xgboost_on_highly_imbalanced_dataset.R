...
# says the tune file generated notes and may cause stacking to fail
# however, the notes are just saying `terms_select()` was deprecated in recipes (previous version)
# this note should hopefully not cause a problem
...
# keep getting a convergence error involving lambda...
# setting non_negative = FALSE in blend_predictions() still does not make the model stack..
ensemble_model =
  data_stack %>%
  blend_predictions(control = control_grid(allow_par = FALSE), penalty = 10^(-3))
