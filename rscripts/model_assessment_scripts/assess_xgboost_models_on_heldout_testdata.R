...
model_paths = system('realpath /vortexfs1/omics/pachiadaki/dgellermcgrath/ensemble/rdata/xgboost_models/*', intern = TRUE)
...
xgb_testdata_conf_matrices = future_pmap(
  .progress = TRUE,
...
xgb_testdata_conf_matrices = xgb_testdata_conf_matrices %>%
  map(~ .x$result) %>%
  set_names(module_names) %>%
  keep(~ !is.null(.x))

xgb_testdata_gen_perf_metrics <- purrr::imap_dfr(
  xgb_testdata_conf_matrices, ~
    .x$conf_mat %>%
...
# avg metrics of 397 ensembles; 19 produced NAs...need to inspect those ones carefully
reduce(
  xgb_testdata_strat %>%
    map(~ .x %>% select(F1, Precision, Recall, `Pos Pred Value`, `Neg Pred Value`)) %>% #397
    keep(~ all(!is.na(.x$F1))), #379
  `+`) %>%
  mutate(across(everything(), ~ .x / 379))

# F1 Precision    Recall Pos Pred Value Neg Pred Value
# 1  0.6501248 0.5447444 0.9219854      0.5447444      0.9683298
# 2  0.8072959 0.7440247 0.9209507      0.7440247      0.9663469
# 3  0.8658744 0.8263768 0.9296006      0.8263768      0.9700044
# 4  0.9000886 0.8714194 0.9431344      0.8714194      0.9750463
# 5  0.9265241 0.9065184 0.9534986      0.9065184      0.9790652
# 6  0.9411998 0.9277435 0.9581045      0.9277435      0.9824126
# 7  0.9548009 0.9482627 0.9634085      0.9482627      0.9846576
# 8  0.9653800 0.9647950 0.9671328      0.9647950      0.9870156
# 9  0.9734007 0.9781135 0.9694233      0.9781135      0.9883893
# 10 0.9807412 0.9911845 0.9711916      0.9911845      0.9893299
...
save(xgb_testdata_strat, xgb_testdata_gen_perf_metrics, xgb_testdata_conf_matrices, file = '/vortexfs1/omics/pachiadaki/dgellermcgrath/ensemble/rdata/auxiliary_r_data/xgboost_models_sparsification_test_results_so_far.rdata')
